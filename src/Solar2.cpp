...
bool Solar2::update(float * inclinations)
{
    ...
    try
    {
        ...
        if(packet[0] != 'O' && packet[1] != 'K'){
            ...
            incs[0] = incs[0]*M_PI/180;
            incs[1] = incs[1]*M_PI/180;

            std::cout << "Xi: " << incs[0] << " Yi: " << incs[1] << std::endl;

            // Convert inclinations to euler angles with convention x'-y' (First rotation around x Axis, then rotation around the new y-axis.)
            float *euler_angles;
            euler_angles = toEuler(incs);

            std::cout << "Xe: " << euler_angles[0] << " Ye: " << euler_angles[1] << std::endl;
...
float *Solar2::toEuler(float *inclinations) {
    float alpha = inclinations[0];
    float beta = inclinations[1];

    float alpha_euler = alpha;
    // Calculations according to Miros Magic 3D-Trigonometry Calculations hopefully soon to be found in the github. If not, complain to me about it.
    float beta_euler = acos(sqrt(1-pow(sin(beta)*tan(alpha), 2.0) - pow(sin(beta), 2)));

    static float euler_angles[2] = {alpha_euler, beta_euler}; 
